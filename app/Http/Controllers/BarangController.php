...
public function checkout(Request $request)
{
    ...
    if (empty($selectedIds)) {
        return redirect()->back()->with('error', 'Silakan pilih barang untuk checkout.');
    }

    // Ambil data keranjang yang dipilih user
    $keranjangDipilih = Keranjang::with('barang')
        ->whereIn('id', $selectedIds)
        ->where('user_id', Auth::id())
        ->get();

    return view('user.checkout', compact('keranjangDipilih'));
}

public function verifikasiCheckout($id)
{
    $checkout = Checkout::findOrFail($id);
    $userId = $checkout->user_id; // ambil ID user dari checkout
    $timestamp = now()->format('YmdHis'); // hasil: 20250630134512
    $resi = $userId . $timestamp;
...
public function showCheckoutForm()
{
    ...
    return view('user.heckcout', compact('keranjangDipilih'));
}
public function prosesCheckout(Request $request)
{
    ...
    foreach ($request->keranjang_id as $id) {
        $keranjang = Keranjang::with('barang')->find($id);
    ...
    return redirect()->route('user.upbukti', ['id' => $request->keranjang_id[0]])
        ->with('success', 'Checkout berhasil, silakan upload bukti pembayaran.');
}
public function formUploadBukti($id)
{
    $checkout = Checkout::where('keranjang_id', $id)
        ->where('user_id', Auth::id())
        ->firstOrFail();

    return view('user.upbukti', compact('checkout'));
}
public function simpanBukti(Request $request, $id)
{
    ...
    if ($request->hasFile('bukti_pembayaran')) {
        ...
        $checkout->bukti_pembayaran = $path;
        $checkout->status = 'menunggu_verifikasi';
        $checkout->save();
    }

    return redirect()->route('user.keranjang')->with('success', 'Bukti pembayaran berhasil diunggah.');
}

public function adminCheckout()
{
    $checkouts = Checkout::with(['user', 'keranjang.barang'])->get();

    return view('admin.datacheckout', compact('checkouts'));
}
